<script lang="ts">
  ...
  async function save(): Promise<void> {
    ...
    try {
      if (creating) {
        ...
        toast.success(`Usuario ${editing.email} actualizado`);
      }
      creating = false;
      editing = null;
      await load();
      ...
      error = e instanceof Error ? e.message : String(e);
      toast.fromError('No se pudo guardar el usuario', e);
    }
  }
  ...
  async function remove(u: User): Promise<void> {
    ...
    try {
      ...
      toast.success(`Usuario ${u.email} eliminado`, {
        description: 'Sus sesiones activas se cerraron.'
      });
    } catch (e: unknown) {
      toast.fromError('No se pudo eliminar el usuario', e);
    }
  }
  ...
  async function setPassword(): Promise<void> {
    ...
    try {
      ...
      error = e instanceof Error ? e.message : String(e);
      toast.fromError('No se pudo cambiar la contraseña', e);
    }
  }
</script>
  ...
  <table>
    ...
    <tbody>
      {#if loading && users.length === 0}
        <SkeletonTable rows={4} cols={5} widths={['30%', '20%', '14%', '20%', '16%']} />
      {/if}
      {#each users as u}
        <tr>
          <td>
