...
pub fn router() -> Router<SharedState> {
    Router::new()
        ...
        .route("/alerts/rules/:id", get(get_rule).put(update_rule).delete(delete_rule))
...
async fn list_rules(
    ...
    let rows: Vec<AlertRuleRow> = state
        .ch
        .select(&format!(
            "SELECT id, project_id, name, description, source, query, condition, threshold, \
             ...
             FROM faro.alert_rules FINAL WHERE deleted = 0{proj} ORDER BY name"
        ))
        .await?;
    Ok(Json(rows))
}
...
fn default_sev() -> String { "warn".into() }
fn default_enabled() -> u8 { 1 }
fn default_project_rule() -> String { "default".into() }
...
async fn get_rule(
    ...
    let sql = format!(
        "SELECT id, project_id, name, description, source, query, condition, threshold, \
         window_seconds, interval_seconds, severity, notification_targets, enabled, \
         created_at, updated_at, deleted, version \
         FROM faro.alert_rules FINAL WHERE id = '{id}' AND deleted = 0 LIMIT 1"
    );
    state
        .ch
        .select_one::<AlertRuleRow>(&sql)
        .await?
        .map(Json)
...
async fn update_rule(
    ...
    let sql = format!(
        "SELECT id, project_id, name, description, source, query, condition, threshold, \
         window_seconds, interval_seconds, severity, notification_targets, enabled, \
         created_at, updated_at, deleted, version \
         FROM faro.alert_rules FINAL WHERE id = '{id}' LIMIT 1"
    );
...
async fn list_incidents(
    ...
    let sql = format!(
        "SELECT id, project_id, rule_id, rule_name, started_at, \
         ...
         WHERE started_at >= toDateTime64('{from}', 3) AND started_at <= toDateTime64('{to}', 3){proj} \
         ORDER BY started_at DESC LIMIT {limit}",
        from = crate::api::params::ch_dt(from),
        to = crate::api::params::ch_dt(to),
        limit = range.limit(),
    );
    let rows: Vec<AlertIncidentRow> = state.ch.select(&sql).await?;
    Ok(Json(rows))
}
